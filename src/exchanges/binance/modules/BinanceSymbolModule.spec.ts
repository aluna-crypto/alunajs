...
describe('BinanceSymbolModule', () => {
  ...
  it('should list Binance raw symbols just fine', async () => {
    ...
    expect(rawSymbols[0].symbol).to.be.eq(BINANCE_RAW_SYMBOLS.symbols[0].symbol)
    expect(rawSymbols[0].baseAsset).to.be.eq(BINANCE_RAW_SYMBOLS.symbols[0].baseAsset)
    expect(rawSymbols[0].quoteAsset).to.be.eq(BINANCE_RAW_SYMBOLS.symbols[0].quoteAsset)

    expect(rawSymbols[1].symbol).to.be.eq(BINANCE_RAW_SYMBOLS.symbols[1].symbol)
    expect(rawSymbols[1].baseAsset).to.be.eq(BINANCE_RAW_SYMBOLS.symbols[1].baseAsset)
    expect(rawSymbols[1].quoteAsset).to.be.eq(BINANCE_RAW_SYMBOLS.symbols[1].quoteAsset)

    expect(rawSymbols[2].symbol).to.be.eq(BINANCE_RAW_SYMBOLS.symbols[2].symbol)
    expect(rawSymbols[2].baseAsset).to.be.eq(BINANCE_RAW_SYMBOLS.symbols[2].baseAsset)
    expect(rawSymbols[2].quoteAsset).to.be.eq(BINANCE_RAW_SYMBOLS.symbols[2].quoteAsset)

...
describe(__filename, () => {
  ...
  it('should parse a Binance raw balance just fine (SPOT)', async () => {
  ...
  it('should parse a Binance raw balance just fine (MARGIN)', async () => {

    // preparing data
    const exchange = new BinanceAuthed({ credentials })

    const rawBalance = BINANCE_RAW_MARGIN_BALANCES[0]
    ...
    expect(balance.total).to.be.eq(total)
    expect(balance.symbolId).to.be.eq(asset)
    expect(balance.wallet).to.be.eq(AlunaWalletEnum.MARGIN)

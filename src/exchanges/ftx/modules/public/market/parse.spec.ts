...
import { mockSplitFtxSymbolPair } from './helpers/splitFtxSymbolPair.mock'
...
describe(__filename, () => {
  ...
  it('should parse a Ftx raw future market just fine', async () => {

    // preparing data
    const rawMarket = clone(FTX_RAW_MARKETS[0])

    rawMarket.name = 'BTC-PERP'
    rawMarket.underlying = 'BTC'
    rawMarket.type = FtxMarketTypeEnum.FUTURE

    const expectedBaseSymbolId = 'BTC'
    const expectedSymbolId = 'BTC'
    ...
    splitFtxSymbolPair.returns({
      ...
      quoteSymbolId: expectedSymbolId,
    })
    ...
    expect(market.derivativesEnabled).to.be.ok

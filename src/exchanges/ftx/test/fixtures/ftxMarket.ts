import { FtxMarketTypeEnum } from '../../enums/FtxMarketTypeEnum'
import { IFtxMarketSchema } from '../../schemas/IFtxMarketSchema'



export const FTX_RAW_MARKETS: IFtxMarketSchema[] = [
  {
    name: 'BNB/USD',
    enabled: true,
    postOnly: false,
    priceIncrement: 0.001,
    sizeIncrement: 0.01,
    minProvideSize: 0.01,
    last: 407.63,
    bid: 407.6,
    ask: 407.63,
    price: 407.63,
    type: FtxMarketTypeEnum.SPOT,
    baseCurrency: 'BNB',
    quoteCurrency: 'USD',
    underlying: null,
    restricted: false,
    highLeverageFeeExempt: true,
    change1h: 0.004462580608596443,
    change24h: -0.014722482059165477,
    changeBod: -0.002027136204946408,
    quoteVolume24h: 48865760.93654,
    volumeUsd24h: 48865760.93654,
    isEtfMarket: false,
  },
  {
    name: 'BTC/USD',
    enabled: true,
    postOnly: false,
    priceIncrement: 1,
    sizeIncrement: 0.0001,
    minProvideSize: 0.0001,
    last: 43401,
    bid: 43403,
    ask: 43404,
    price: 43403,
    type: FtxMarketTypeEnum.SPOT,
    baseCurrency: 'BTC',
    quoteCurrency: 'USD',
    underlying: null,
    restricted: false,
    highLeverageFeeExempt: true,
    change1h: 0.004768849688635785,
    change24h: -0.01947362475996837,
    changeBod: -0.009290116411778133,
    quoteVolume24h: 655827136.8962,
    volumeUsd24h: 655827136.8962,
    isEtfMarket: false,
  },
  {
    name: 'ETH/USD',
    enabled: true,
    postOnly: false,
    priceIncrement: 0.1,
    sizeIncrement: 0.001,
    minProvideSize: 0.001,
    last: 2911,
    bid: 2910.9,
    ask: 2911,
    price: 2911,
    type: FtxMarketTypeEnum.SPOT,
    baseCurrency: 'ETH',
    quoteCurrency: 'USD',
    underlying: null,
    restricted: false,
    highLeverageFeeExempt: true,
    change1h: 0.009677083694634247,
    change24h: -0.031700096464092074,
    changeBod: -0.009695526450076543,
    quoteVolume24h: 400900371.3091,
    volumeUsd24h: 400900371.3091,
    isEtfMarket: false,
  },
]



export const FTX_RAW_FUTURE_MARKET: IFtxMarketSchema = {
  name: 'BTC-PERP',
  enabled: true,
  postOnly: false,
  priceIncrement: 1,
  sizeIncrement: 0.0001,
  minProvideSize: 0.001,
  last: 30314,
  bid: 30314,
  ask: 30315,
  price: 30314,
  type: FtxMarketTypeEnum.FUTURE,
  baseCurrency: null,
  isEtfMarket: false,
  quoteCurrency: null,
  underlying: 'BTC',
  restricted: false,
  highLeverageFeeExempt: true,
  change1h: 0.01188330329127445,
  change24h: -0.014339131848479921,
  changeBod: 0.02005518540951612,
  quoteVolume24h: 4425672230.3978,
  volumeUsd24h: 4425672230.3978,
}
